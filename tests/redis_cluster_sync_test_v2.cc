...
{
    m_pClusterCtx = redisClusterContextInit();
}
                                       ...
                                       unsigned short>& mpRedisNodes)
{
    ...
    m_pClusterCtx = redisClusterContextInit();
    redisClusterSetOptionAddNodes(m_pClusterCtx, strRedisNode.c_str());
    redisClusterConnect2(m_pClusterCtx);
    if (m_pClusterCtx == NULL || m_pClusterCtx->err)
    {
        std::cout << "conn fail, err msg: " << m_pClusterCtx->errstr << "\n";
        return ;
    }
